...
pub fn create_invoice(client_service: &ClientService, invoice_service: &InvoiceService, user: &User) -> Result<(), AppError> {
...
pub fn list_invoices(invoice_service: &InvoiceService) -> Result<(), AppError> {
        ...
        .interact()?
    {
        ...
        let mut invoice_options: Vec<String> = invoices.iter().map(|i| format!("Invoice #{} - {}", i.id, i.date)).collect();
