...
pub fn create_client(client_service: &ClientService) -> Result<(), AppError> {
    ...
    let cif = loop {
        ...
        match validate_cif(&cif_input) {
            ...
            Err(e) => {
                ...
                println!(
                    "{}",
                    style("CIF/NIF must be between 8-12 characters").yellow()
                );
                continue;
            }
        }
    ...
    let email = loop {
        ...
        match validate_email(&email_input) {
            ...
            Err(e) => {
                ...
                println!(
                    "{}",
                    style("Please enter a valid email format (user@domain.com)").yellow()
                );
                continue;
            }
        }
    ...
    let client = client_service
        .create_client(name, cif, address, email)
